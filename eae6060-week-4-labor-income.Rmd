...
date: "2022/3 - Topic 4: Optimal labor taxation"
output:
...
knitr::include_graphics("figs/aula-4-grafico-27.png")
```

A rise in marginal tax after $z^*$ has two effects on revenue: individuals are *mechanically* paying more taxes for a fixed taxable income (positive), but for a given tax level they *behaviorally* reduce their income (negative) &mdash; at the optimum, these effects must be equal `r Citep(myBib, "piketty2013optimal")`
...
But elasticities of labor supply empirically are very small, around 0.1 `r Citep(myBib, "keane2011labor")`, which generate very high optimal tax rates &mdash; in that case, the top of Laffer curve is at $91\%$
...
knitr::include_graphics("figs/aula-4-grafico-3.png")
```
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figs/aula-4-grafico-4.png")
```
...
knitr::include_graphics("figs/aula-4-grafico-5.png")
```
...
```{r, echo=FALSE, out.width = '80%'}
knitr::include_graphics("figs/aula-4-grafico-7.png")
```
...
```{r, echo=FALSE, out.width = '90%'}
knitr::include_graphics("figs/aula-4-grafico-6.png")
```
...
`r Citep(myBib, "bertrand2001ceos")` show that CEOs frequently earn higher payments for reasons entirely outside of their control ("luck") &mdash; this would imply that their remuneration does not only reflect high produtivity, but also *bargaining for rents*
...
```{r, echo=FALSE, out.width = '90%'}
knitr::include_graphics("figs/aula-4-grafico-9.png")
```
...
knitr::include_graphics("figs/aula-4-grafico-10.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-4-grafico-11.png")
```
...
```{r, echo=FALSE, out.width = '90%'}
knitr::include_graphics("figs/aula-4-grafico-12.png")
```
...
knitr::include_graphics("figs/aula-4-grafico-13.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-4-grafico-14.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-4-grafico-15.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-4-grafico-16.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-4-grafico-17.png")
```
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figs/aula-4-grafico-18.png")
```
...
```{r, echo=FALSE, out.width = '90%'}
knitr::include_graphics("figs/aula-4-grafico-19.png")
```
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figs/aula-4-grafico-20.png")
```
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figs/aula-4-grafico-21.png")
```
...
```{r, echo=FALSE, out.width = '80%', fig.align='center'}
knitr::include_graphics("figs/aula-4-grafico-22.png")
```
...
```{r, echo=FALSE, out.width = '90%'}
knitr::include_graphics("figs/aula-4-grafico-23.png")
```
...
Bunching: literatura que tenta estimar elasticidade da renda tributável olhando como contribuintes se agrupam em quinas (*kinks*) na curva de imposto marginal

O grande insight é que sob comportamento racional, a quantidade de contribuintes se agrupando na quina informa sobre a taxa marginal de substituição entre consumo e lazer

Heuristicamente, se a taxa marginal sobe de $60\%$ para $80\%$, vão se agrupar todos (e apenas eles) cujo o custo (em termos de menor lazer) de trabalhar `$`1 extra é maior que $0.6$ mas menor que $0.8$

---
class: middle
## Contribuição

O insight do bunching gerou uma longa literatura, mas tem problemas: ele é drasticamente dependente de comportamento bastante racional e mercado sem fricções

No mundo real é muito mais difícil controlar a renda tributável, especialmente para trabalhadores, o que limita as possibilidades de bunching

Além disso, o custo de bem-estar de não fazer bunching é pequeno (de 2a ordem), e há evidências que pessoas fazem bunching só no maior dos *kinks* (Mortenson `&` Whitten, 2020)

---
class: middle
## Contribuição (II)

Esse artigo analisa o papel das fricções no mercado de trabalho para constranger o bunching

Usa dados da Dinamarca, e mostra que horas são definidas coletivamente em geral, mas há uma tendência das negociações coletivas fazerem um "bunching agregado"

Também argumenta que com custos de ajustamento (realista!), alterações maiores nos impostos geram elasticidades estimadas maiores nos dados

Discute o efeito dessas fricções em gerar diferenças em elasticidade "macro" (razoavelmente altas e iguais à estrutural) e elasticidades "micro" (perto de zero por causa dos custos de ajustamento)

---
class: middle
## Modelo

Apresenta um modelo de oferta de trabalho com rigidez nas horas trabalhadas e custos de ajustamento

Predições do modelo:

1. Quando há custos de procura e rigidez nas horas, a elasticidade observada cresce com o tamanho da quina (e "em OLS" pelo tamanho da reforma)
2. Por causa de restrições de horas e custos de procura, haverá "bunching agregado" perto dos kinks que se apliquem a uma maioria
3. A quantidade de bunching agregado e individual é positivamente correlacionada ao longo das ocupações

---
class: middle

```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-22.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-23.png")
```
...
Considere uma função de utilidade quasilinear CES, que gera uma demanda por trabalho $h_i^* = \alpha_i \left( (1 - \tau)w \right)^{\epsilon}$ quando frente a uma restrição orçamentária (taxação) linear

Se a taxação é *piecewise-linear*, com taxas marginais $\tau_1$ e $\tau_2$, então existe $\underline{\alpha}$ e $\bar{\alpha}$ tais que:

$$h_i^* = \alpha_i \left( (1 - \tau_1)w \right)^{\epsilon} \text{ se } \alpha < \underline{\alpha},$$

$$h_i^* = \alpha_i \left( (1 - \tau_2)w \right)^{\epsilon} \text{ se } \alpha > \bar{\alpha}$$

e $h_K = K/w$ se $\alpha \in \left[ \underline{\alpha}, \bar{\alpha} \right]$, onde $\underline{\alpha} = h_K /\left( (1 - \tau_1)w \right)^{\epsilon}$ e $\bar{\alpha} = h_K / \left( (1 - \tau_2)w \right)^{\epsilon}$

---
class: middle
## Bunching (II)

Então a densidade de quem faz bunching, $B_{NL} = F(\bar{\alpha}) - F(\underline{\alpha})$

$$= F(h_K / \left( (1 - \tau_2)w \right)^{\epsilon}) - F(h_K /\left( (1 - \tau_1)w \right)^{\epsilon})$$

Fazendo uma aproximação de primeira ordem (Taylor):
...
$$\Rightarrow \frac{B_{NL}}{h_K} = \frac{g(\alpha_i)}{2} \left[ \left( \frac{(1 - \tau_1)}{(1 - \tau_2)} \right)^{\epsilon} - 1 \right]$$
...
Para estimar a densidade contrafactual, ele estima um polinomial de ordem $q$ perto (mas fora) do entorno do kink, dividindo em bins $j$ e estimando: $$C_j = \sum_{i=0}^{q} \beta_i^0 (Z_j)^i + \sum_{i=-R}^{R} \gamma_i^0 \mathbb{1} \left[Z_j = i \right] + \varepsilon_j^0$$

Se $\hat{C}_j = \sum_{i=0}^{q} \beta_i^0 (Z_j)^i$ é o polinomial retirando a área ao redor da quina, então $\hat{B}_N^0 = C_j - \hat{C}_j = \sum_{i=-R}^{R} \gamma_i^0$

E daí calculamos $$\hat{b} = \frac{\hat{B}_N}{\sum_{j=-R}^{R} \hat{C}_j / (2R + 1)}$$

O erro padrão pode ser mais facilmente calculado por bootstrap

---
class: middle

```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-24.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-25.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-26.png")
knitr::include_graphics("figs/aula-5-grafico-27.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-33.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-28.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-29.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-30.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-31.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-34.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-35.png")
```
...
```{r, echo=FALSE, out.width = '100%'}
knitr::include_graphics("figs/aula-5-grafico-32.png")
```
